...
def get_client_ip():
    # Check X-Forwarded-For for proxies (common in platforms like Heroku)
    ...
    if x_forwarded_for:
        ...
        for ip in ip_list:
            if not is_private_ip(ip):  # Ensure we only accept non-private IPs
                return ip
        return ip_list[-1]  # If all are private, use the last one (likely the proxy IP)
    else:
        return request.remote_addr  # Fallback to the remote address
